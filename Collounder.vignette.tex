...
In this second section, we are going to analyze the problems and biases you can
create by not controlling a particular case of covariates: the colliders. When
studying the effects of differents variables on a particular one, and the 
difficulties of adjusting by the collider, Z, when X and Y are its parents and
may or may not have an association between them. In this case Z is the \textbf{common effect}
of X and Y.
We have used the following modules to make the analysis:
...
Let's look at our data one more time and see whether this correlation we find when adjusting
by our collider (speed) is actually present. The second plot shows the correlation that actually exists between the variables cholesterol and heart, as opposed to the previous ones (for comparison).
...
We want to study the effect of UV radiation (hours of exposure a day) and INK4a on the mutation of gen p53. 
...
Again we find no correlation between cortisol and cholesterol, even if we adjust for the ancestor (sugar_consumption).
